...
void enemyInit(Enemy enemy[], Vector2 playerPos, int type, int scoreMilestone) {
  for (int i = 0; i < ENEMY_NUM; i++) {
    if (!enemy[i].active) {
      ...
      Vector2 enemySpawnPosition = {GetRandomValue(150, 2500),
                                    GetRandomValue(150, 1300)};

      while (CheckCollisionCircleRec(
          playerPos, distanceFromPlayerRadius,
          (Rectangle){enemySpawnPosition.x, enemySpawnPosition.y, 50, 50})) {
        enemySpawnPosition =
            (Vector2){GetRandomValue(150, 2500), GetRandomValue(150, 1300)};
      }
      enemy[i].type = type;
      enemy[i].pos = enemySpawnPosition;
      enemy[i].rec = (Rectangle){enemy[i].pos.x, enemy[i].pos.y, 50, 50};
      enemy[i].dir = Vector2Normalize((Vector2){playerPos.x - enemy[i].pos.x,
                                                playerPos.y - enemy[i].pos.y});
      enemy[i].active = true;
...
void enemyFollowPlayer(Enemy enemy[], Vector2 playerPos, int i) {
  if (!enemy[i].reachedFollowDir) {
    ...
    enemy[i].dir = Vector2Normalize(
        (Vector2){point.x - enemy[i].pos.x, point.y - enemy[i].pos.y});
...
int enemyUpdate(Enemy enemy[], Rectangle playerRec, Weapon axe,
                Vector2 playerPos, Rectangle rec[], int recNum,
                ParticleSystem *ps) {
  int returnValue = 0;
  for (int i = 0; i < ENEMY_NUM; i++) {
    if (enemy[i].active) {
      ...
      if (CheckCollisionRecs(enemy[i].rec, axe.rec) &&
          enemy[i].state == NOT_HIT && (axe.state == 1 || axe.state == 3)) {
...
void enemyAttackUpdate(Enemy enemy[], Vector2 playerPos, int i) {
  ...
  if (!enemy[i].isAttacking &&
      CheckCollisionCircleRec(playerPos, enemyMeleeAttackRadius,
                              enemy[i].rec) &&
      !enemy[i].inAttackCooldown) {
...
void enemyDraw(Enemy enemy[], bool upgradeState) {
  for (int i = 0; i < ENEMY_NUM; i++) {
    if (enemy[i].active) {
      ...
      Rectangle animRec = {enemy[i].rec.x - 25, enemy[i].rec.y - 25,
                           enemy[i].rec.width + 50, enemy[i].rec.height + 50};

      if (enemy[i].isAttacking) {
        DrawRectangleRec(
            enemy[i].attackRec,
            (Color){enemy[i].healthBarColor.r, enemy[i].healthBarColor.g,
                    enemy[i].healthBarColor.b, enemy[i].attackFrameTimer * 4});
      }
      int animDir;
      if (!enemy[i].state == HIT) {
        if (enemy[i].dir.x >= 0) {
        ...
        if (!upgradeState)
          ...
          drawAnimationFrame(&enemy[i].anim, animRec, animDir, 1);
      ...
      DrawRectangle(
          enemy[i].rec.x, enemy[i].rec.y - 50,
          (int)(enemy[i].rec.width * (enemy[i].health / enemy[i].baseHealth)),
          12.5, enemy[i].healthBarColor);
    }
  }
}
